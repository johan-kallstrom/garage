...
class VecWorker(DefaultWorker):
    ...
    def __init__(self, *, seed, max_path_length, worker_number, n_envs=8):
    ...
    def update_env(self, env_update):
        ...
        if isinstance(env_update, list):
            ...
            for env_index, env_up in enumerate(env_update):
                self._udpate_env_inner(env_up, env_index)
        elif env_update is not None:
            for env_index in range(self._n_envs):
                self._udpate_env_inner(copy.deepcopy(env_update), env_index)

    def _udpate_env_inner(self, env_update, env_index):
